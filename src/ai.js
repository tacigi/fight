/* ============================================================
   ...
   Menggunakan 4 tipe serangan: punchLight, punchHeavy,
   kickLight, kickHeavy + special + ultimate.
   ============================================================ */
...
export class CpuController {
  ...
  decide(self, opponent) {
    ...
    if (dist > 220) {
      ...
      const r = Math.random();
      if (r < 0.12) return { hold: null, press: this.controls.special[0], duration: 24 };
      if (r < 0.5)  return { hold: null, press: this.controls.kickHeavy[0], duration: 30 };
      if (r < 0.8)  return { hold: null, press: this.controls.kickLight[0], duration: 20 };
      return { hold: 'forward', press: null, duration: 20 + Math.random() * 20 };
    }

    if (dist > 120) {
      const r = Math.random();
      if (r < 0.3) return { hold: 'forward', press: null, duration: 14 + Math.random() * 14 };
      if (r < 0.5) return { hold: null, press: this.controls.kickLight[0], duration: 20 };
      if (r < 0.7) return { hold: null, press: this.controls.kickHeavy[0], duration: 28 };
      if (r < 0.9) return { hold: null, press: this.controls.punchHeavy[0], duration: 24 };
    ...
    if (r < 0.4)  return { hold: null, press: this.controls.punchLight[0], duration: 11 };
    if (r < 0.65) return { hold: null, press: this.controls.punchHeavy[0], duration: 24 };
    if (r < 0.8)  return { hold: null, press: this.controls.kickLight[0], duration: 20 };
    if (r < 0.95) return { hold: null, press: this.controls.special[0], duration: 24 };
    return { hold: 'back', press: null, duration: 14 };
  }
